...
const ExerciseFiltering = (props) => {
  ...
  const handleCheckboxChange2 = (event, exercise) => {
    if (event.target.checked) {
      ...
      const exerciseWithDaySetsReps = { ...exercise, dayOfWeek: selectedDay, sets:sets, reps:reps};
...
const postExerciseData = async (exerciseData) => {
  ...
  const postSession = async (exerciseData) => {
    try {
      ...
      if (!response.ok) {
        // Handle error, throw an exception, or return a meaningful response
        throw new Error(`Failed to post session variable. Status: ${response.status}`);
      }
      ...
      <div style={{ margin: '16px' }}>
        ...
        <FormControl sx={{ minWidth: 30, marginRight: '16px' }}>
            <TextField
                        ...
                        type="number"
                        min="1"
                        value={sets} // Set the default value based on your actual data
        ...
        <FormControl sx={{ minWidth: 30, marginRight: '16px' }}>
            <TextField
                        ...
                        type="number"
                        min="1"
                        value={reps} // Set the default value based on your actual data
